...
plot_predictive_range <- function(p, title = "production", gridsize = 100) {
  p <- c(0.8923841, 0.1076159,    0)  # current WTA strategy production
  p <- c(0.9, 0.09,    0.01)          # previous "intermediate strategy" production
  # p <- c(0.6, 0.4, 0)
  p <- LLM_pred_inter_narrow[1,]
  gridsize = 300
  ...
  grid_ae <- expand.grid(
    alpha = seq(0.001,2, length.out = gridsize),
    epsilon = seq(0,1, length.out = gridsize)
  ) |> as_tibble() 
  pred_matrix = matrix(0, nrow = nrow(grid_ae), ncol = 3)
  for (i in 1:nrow(pred_matrix)) {
    # V1: epsilon-random error
    q <- p^grid_ae$alpha[i] / sum(p^grid_ae$alpha[i])
    pred_matrix[i,] <- (1-grid_ae$epsilon[i]) * q + grid_ae$epsilon[i] * c(1/3,1/3,1/3)
    # V2: smooth utilities
    # pPrime <- p + grid_ae$epsilon[i]
    # pred_matrix[i,] <- pPrime^grid_ae$alpha[i] / sum(pPrime^grid_ae$alpha[i])
    # V3: SM |> + eps |> normalize (trivial ::: all predictions covered)
    # q <- p^grid_ae$alpha[i] / sum(p^grid_ae$alpha[i]) + grid_ae$epsilon[i]
    # pred_matrix[i,] <- q/sum(q)
  }
...
p_prod <- c(0.8923841, 0.1076159,    0) # WTA strategy
prediction_range_prod  <- plot_predictive_range(p_prod, "production", gridsize = 10)
prediction_range_prod

# p_inter <- prob_inter[1,] / sum(prob_inter[1,])
# p_inter <- c(0.681758393, 0.316123548, 0.002118059)
p_inter <- c(0.6771523, 0.3228477,    0) # WTA strategy
prediction_range_inter <- plot_predictive_range(p_inter, "interpretation", gridsize = 10)
