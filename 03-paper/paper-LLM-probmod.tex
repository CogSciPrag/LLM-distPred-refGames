...
Common to these approaches is that they derive or define, based on some explicit conceptual motivation, parameterized stochastic policy, $P_{S}(u \mid s; \theta_{S})$ for a speaker's choice of expression or utterance $u$ given a referent or state $s$ to describe, modulated by parameters $\theta_{S}$; and a stochastic listener policy, $P_{L}(s \mid u; \theta_{L})$, capturing the probability of choosing a referent $s$ for utterance $u$.
As a concrete example, we introduce the Rational Speech Act (RSA) model first introduced in this form by \citet{FrankGoodman2012:Predicting-Prag} \citep[for overview see][]{FrankeJager2015:Probabilistic-p,GoodmanFrank2016:Pragmatic-Langu,StevensBenz2018:Game-Theoretic-,ScontrasTessler2021:A-practical-int,Degen2023:The-Rational-Sp}.
...
\end{align*}
%
The softmax function takes a vector of non-normalized scores $\myvec{s}$ and outputs a probability distribution in which odds are proportional to score differences, modulated by optimality parameter $\alpha$:
%
\begin{align*}
 \text{SoftMax}(\myvec{s}, \alpha) = \myvec{p}, \text{where } p_{i} \propto \expo \left (\alpha p_{i} \right )
\end{align*}
%
Finally, the pragmatic listener is defined as the policy resulting from applying Bayes rule, solving the inverse-problem for the previously define speaker policy:
%
\begin{align*}
...
Yet, the model also predicts probability zero for distractor choices, so that the human data, where the distractor option was chosen in both conditions, would immediately rule out the model entirely.
...
\mf{two-parameters, can be fitted by Bayes, results in Figure ...}
...
\begin{figure}
  ...
  The notation for the condition-level options uses pseudo-code to more clearly reflect the relevant operator scope differences (where triangles represent the pipe operator). \mf{include dots in mean function, too}}
...
%
\begin{align*}
  P(R_{l}, , C; \alpha_{c} \epsilon_{c}) \propto P_{m}(R_{l}, C; \alpha_{c}) +  \frac{\epsilon_{c}}{3}
\end{align*}
