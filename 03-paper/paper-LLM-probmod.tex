...
\begin{figure}
  \centering
    ...
    \includegraphics[width=0.45\linewidth]{00-pics/refgame-counts.pdf}
    \hfill
    \includegraphics[width=0.45\linewidth]{00-pics/model-predictions-vanilla.pdf}

  \caption{Counts of choices from reference games with human participants (left-hand side), alongside vanilla model predictions from GPT-3.5 (right-hand side). \mf{fix plots}}
...
Since the reference game descriptions and choice options are all text-based, probabilities for discrete-choice predictions of LLMs can be defined as a function of the (log) probabilities assigned to the continuations corresponding to each choice option after processing the game description.
...
Using average log-probs for each answer option as a basic score, there are at least three conceivable ways of deriving forced-choice probabilities when averaging over item-level variation. \mf{include figure}
...
\end{align*}
%
This means that there are three conceivable scope-sites for aggregating item-level information as shown in Figure XYZ. \mf{make figure}.
...
\begin{align*}
  ...
  & P_{w}(R_{l}, C) \propto  \left [ \frac{1}{m} \ \sum_{i = k}^{m} \frac{\expo \left( S(R_{l}, I_{k}) \right )}{ \sum_{l'} \expo \left( S(R_{l'}, I_{k}) \right )} \right ]^{\alpha}
\end{align*}
%

\mf{explain conceptual differences}

These different conditional-level predictions do not, generally, coincide.
For example, the right-hand side of Figure~\ref{fig:refgame-counts} shows model predictions for $\alpha=1$, which is the special case where the intermediate- and wide-scope approach do coincide.
