...
The prevalent approach to assessing LLM performance based on benchmark-testing quantifies accuracy for a multiple-choice task with a ``winner-takes-all'' (WTA) strategy.
...
%
\begin{align*}
  S_{\text{item}}\left( y_{ki}, x_{k} \right) = \log P_{\text{LLM}} \left(y_{ki} \mid x_{k} \right) =  \sum_{j=1}^{|y_{ki}|} \log P_{\text{LLM}} \left(w_{kij} \mid x_{k}, w_{ki1}, \dots, w_{ki(j-1)} \right)  \,.
\end{align*}
%
The WTA approach considers item $I_{k}$ to be answered correctly if the designated ``goal answer'' $y_{ki^{*}}$ is among the options that maximize the item-level score.
The accuracy of the LLM on the given task is the proportion of items in which the correct answer is selected. \mf{Rephrase or add FN on tie-breaking.}
...
%
\begin{align*}
P_{\text{choose}}(y_{ki}) \propto \expo \left [ \alpha \ S_{\text{item}}\left( y_{ki}, x_{k} \right) \right ]\,,
\end{align*}
%
...
Imagine that there are two options, and that the target option is a small $\epsilon$ better in 80\% of the task's items, and otherwise worse.
...
If the remaining 20\% of the items are such that the non-goal option is almost infinitely better, it would be chosen under a pure sampling strategy, where $\alpha = 1$, with virtual certainty, so the softmax-based accuracy would around 0.4.\footnote{The probability of the second option in the 80\% of items where the goal answer is slightly better is 0.5 in the limit of $\epsilon \rightarrow 0$, and it is virtually 1 in the remaining 20\%.}
...
Differences in accuracy scores between a WTA-based or a more general softmax-based strategy depend on the properties of the variation in item-level scores, in particular the relation between score-ordering and score-differences.
(The example was set up deliberately to have one score-ordering for very small score-differences, and another for huge score-differences.)
...
Data from forced-choice task with human participants will likely show item-level variation, which is why hierarchical modeling accommodating item-level differences is recommended in statistical analyses \citep[e.g.,][]{Jaeger2008:Categorical-dat,barr2013}.
...
\mf{To address whether LLM's condition-level predictions are adequate as item-level predictors, I should re-run the item-level analysis with the mean posterior predictive of the intermediate-scope model (instead of, or in addition to RSA)!}
