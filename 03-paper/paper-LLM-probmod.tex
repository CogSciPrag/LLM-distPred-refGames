...
In sum, the condition-level predictions of the RSA model are a parameterized function $P_{\text{cond}}^{\text{RSA}}(R_{l}, C; \alpha_{c})$, assigning a probability to each response category $R_{l}$ in each condition $C$ (production or interpretation).
...
This is clearly not the case for the RSA model with posterior predictive $p$-values close to 0.5, as shown in Table~\ref{tab:sumStats}.
...
\end{align*}
%
Fitting for the partitioned data set, and estimates of posterior predictive $p$-values that do not discredit the model (see Table~\ref{tab:sumStats}).
...
\begin{tabular}{llllcrlcrcl}
  ...
  data & condition & model & $|$95\%\ & mean\ & 95\%$|$\ & $|$95\%\ & mean\ & 95\%$|$\ & Bpppv & \\
  \midrule

  cond.-level & avg. scores        & production     & 0.19 & 0.00 & 0.22 & 0.04 & 0.26 & 0.11 & 0.05 & * \\
  cond.-level & avg. scores        & interpretation & 0.15 & 0.00 & 0.18 & 0.02 & 0.20 & 0.06 & 0.00 & * \\
  cond.-level & avg. probabilities & production     & 0.86 & 0.08 & 1.04 & 0.13 & 1.22 & 0.18 & 0.51 &  \\
  cond.-level & avg. probabilities & interpretation & 0.36 & 0.00 & 0.48 & 0.05 & 0.60 & 0.12 & 0.00 & * \\
  cond.-level & avg. WTA           & production     & 0.87 & 0.08 & 1.04 & 0.13 & 1.23 & 0.17 & 0.49 &  \\
  cond.-level & avg. WTA           & interpretation & 0.17 & 0.10 & 0.39 & 0.15 & 0.61 & 0.20 & 0.26 &  \\
  cond.-level & RSA                & production     & 2.59 & 0.08 & 3.16 & 0.13 & 3.71 & 0.18 & 0.49 &  \\
  cond.-level & RSA                & interpretation & 0.24 & 0.10 & 0.66 & 0.15 & 1.07 & 0.20 & 0.52 &  \\
  item-level  & LLM                & production     & 0.15 & 0.00 & 0.21 & 0.10 & 0.29 & 0.21 & 0.00 & * \\
  item-level  & LLM                & interpretation & 0.08 & 0.00 & 0.13 & 0.17 & 0.18 & 0.33 & 0.00 & * \\
  item-level  & RSA                & production     & 2.62 & 0.08 & 3.15 & 0.13 & 3.69 & 0.17 & 0.34 &  \\
  item-level  & RSA                & interpretation & 0.24 & 0.10 & 0.66 & 0.15 & 1.05 & 0.20 & 0.25 & \\
...
\caption{
  ...
  Columns show the means and 95\% credible intervals for posterior samples for each parameter, as well as the Bayesian posterior predictive $p$-value for each model, condition and data level.
  Asterisks mark $p$-values not above $0.05$, flagging relative inability to capture the structure of the data.
}
\label{tab:sumStats}
\end{table}
