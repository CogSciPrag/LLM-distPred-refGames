...
To assess goodness-of-fit, we can use the posterior predictive distribution, i.e., the model's predictions about data of the same size and structure as the training data.
...
We see that the RSA model passes this ``visual posterior predictive check'' \citep{Kruschke2021:Bayesian-Analys} for both conditions.
...
In contrast, using the global, item-independent RSA model predictions to fit the item-level data, we obtain estimates of posterior predictive $p$-values that do not discredit the model ($\sim 0.338$ for production, $\sim 0.252$ for interpretation).
...
This section explores different ways of deriving a probabilistic likelihood function for the human data from a Large Language Model at the aggregate, condition-level.
...
Using priors and Bayesian posterior estimation as previously for the RSA (see Section~\ref{sec:model-pred-from}), we obtain samples from the posterior over parameters and posterior predictive distributions.
...
What is noteworthy about these estimates is that the $\alpha_{c}$ parameters for the wide-scope model have a very large credible range in both conditions.
