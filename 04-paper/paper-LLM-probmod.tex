...
\renewcommand{\shorttitle}{LLMs as statistical models for categorical experimental data}
...
The way in which LLM capabilities are assessed depends on what the intended application is.
...
Benchmark-driven assessments are very useful for engineering purposes, when the main issue is to whether a given system can perform a particular task correctly.
...
We introduce different ways of building a Bayesian probabilistic model around core predictor values derived from various LLMs.
...
Rather than aiming for scale and large-coverage, we focus on transparency and zoom in on a single, small-scale case of pragmatic language production and interpretation, for which a widely used probabilistic cognitive model exists, the Rational Speech Act model \citep{FrankGoodman2012:Predicting-Prag}, so that we can compare it against the LLM-fueled statistical models.

The main results of our investigation are, first of all, that it is possible to build Bayesian statistical models around LLM-based predictors, and that some but not all ways of doing so yield models that adequately predict the human data.
Importantly, this investigations highlights that LLMs are by design fundamentally different from common statistical or probabilistic cognitive models, in that LLMs make predictions for each individual item, rather than specifying a predictor of central tendency at a more aggregate level (referred to here as the condition-level; see Section~\ref{motivation}).
This observation raises two important conceptual and empirical questions.
...
We find that variability predicted by the LLM at the item-level is not borne out by the human data, that some but not all ways of constructing condition-level predictions are equally good, and that different LLMs as backends may prefer to use different aggregation strategies.
Our conceptual comparison of LLMs and PCMs also identifies a sense in which the latter are explanatory, while the former are not. \mf{double-check main take away}.
...
Section~\ref{experiment-reference-games} looks at human data from a simple but non-trivial forced-choice experiment, namely a text-based reference game, for which Section~\ref{sec:model-pred-from} introduces a commonly used, theoretically-motivate probabilistic cognitive model, namely the Rational Speech Act (RSA) model \citep{FrankGoodman2012:Predicting-Prag}.
...
\mf{include section to extend to LLaMA}
