...
\begin{figure}
  ...
  \caption{Counts of choices from reference games with human participants (left-hand side), alongside vanilla model predictions from GPT-3.5 (right-hand side).}
...
Section~\ref{sec:vanilla-predictions} first describes ``vanilla predictions'' derived from an LLM.
Subsequent Sections~\ref{adding-a-link-function-the-alpha-model} and \ref{sec:adding-noise-the-alpha-epsilon-model} use these vanilla predictions to build slightly more complex statistical models.
All models will be tested for their predictive adequacy, using Bayesian posterior predictive checks.
...
The previous literature has shown that scoring longer sequences of text for an autoregressive LLM in term of ``length corrected log probabilities,'' or more crisply: average next-word surprisals, gives improved predictions in relevant benchmark tests \citep[e.g.,][]{BrownMann2020:Language-Models}.
