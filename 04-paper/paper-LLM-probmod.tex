...
\begin{figure}[t]
  ...
  \caption{Schematic representation of key conceptual differences between different types of statistical models. \mf{picture is cropped}}
...
\mf{metion (somewhere) where item-level effects could be added!}
...
The most obvious \emph{item-level score} an (autoregressive) LLM provides for each choice option $y_{ki}$ is its log-probability:\footnote{
  ...
  \mf{check whether this is correct also after new LLaMA results.}
}
%
\begin{align*}
